...
public class thread_applique extends Thread {
	...
	public thread_applique (JLabel p_status,
	...
	public void run(){
		...
		String cmdEnvoi2=String.format("cmd.exe /c netsh interface ip set address name=\"%s\" gateway=%s gwmetric=0 >resultat.txt", comboBox.getModel().getElementAt(0),passerelle.getText());
		String cmdEnvoi3=String.format("cmd.exe /c netsh interface ip set dns name=\"%s\" source=static addr=%s >resultat.txt", comboBox.getModel().getElementAt(0),dns1.getText());
		String cmdEnvoi4 = String.format("cmd.exe /c netsh interface ip add dns name=\"%s\" addr=%s >resultat.txt", comboBox.getModel().getElementAt(0),dns2.getText());
		...
		try {
			...
			for (String cmd : lstCmd){
				...
				switch (idx){
					case 0:
						status.setText("configuration de l'adresse ip et du masque de sous reseau");
		...
		if (ipApplique&&passerelleApplique&&dns1Applique&&dns2Applique){//tous les parametres ont été appliqué correctement
			
		}else{
			JOptionPane.showMessageDialog(null, "le profil demandé n'a pas été appliqué correctement","Erreur", JOptionPane.WARNING_MESSAGE, null);
			
		}
		
		status.setText("");
		
		thread_lireIpConfig t = new thread_lireIpConfig(comboBox, ip, subMask, passerelle, dns1, dns2);
		t.start();
	...
	private void verifiStatusApplication(int p_index) {
		...
		switch (p_index){
			case 0:
				if (isOk){
					BtIp.setIcon(new ImageIcon(getClass().getResource("/testbal/ok.png")));	
					ipApplique=true;
				
				}else{
					BtIp.setIcon(new ImageIcon(getClass().getResource("/testbal/erreur.png")));
				}
				BtIp.setVisible(true);
				break;
			case 1:
				if (isOk){
					BtPasserelle.setIcon(new ImageIcon(getClass().getResource("/testbal/ok.png")));	
					passerelleApplique=true;
				}else{
					BtPasserelle.setIcon(new ImageIcon(getClass().getResource("/testbal/erreur.png")));
				}
			...
			case 2:
				if (isOk){
					BtDns1.setIcon(new ImageIcon(getClass().getResource("/testbal/ok.png")));	
					dns1Applique=true;
				}else{
					BtDns1.setIcon(new ImageIcon(getClass().getResource("/testbal/erreur.png")));
				}
				BtDns1.setVisible(true);
				break;
			case 3:
				if (isOk){
					BtDns2.setIcon(new ImageIcon(getClass().getResource("/testbal/ok.png")));	
					dns2Applique=true;
				}else{
					BtDns2.setIcon(new ImageIcon(getClass().getResource("/testbal/erreur.png")));
				}
