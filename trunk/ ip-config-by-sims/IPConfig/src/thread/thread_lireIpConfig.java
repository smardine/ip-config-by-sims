...
public class thread_lireIpConfig extends Thread {
	...
	public thread_lireIpConfig(JComboBox p_comboBox, JTextField p_ip,
			JTextField p_subMask, JTextField p_passerelle, JTextField p_dns1,
			JTextField p_dns2) {
	...
	public void run() {
		...
		String cmdEnvoi = String.format("cmd.exe /u /c  ipconfig/all >%s",
				"ipconfig.txt");
		try {
		...
		for (String ligne : lst) {
			if (ligne.contains("Connexion")) {
				...
				comboBoxModel.addElement(ligne.substring(ligne
						.lastIndexOf("Connexion"), ligne.lastIndexOf(":")));
